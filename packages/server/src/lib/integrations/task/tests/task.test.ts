...
describe('Task', () => {
    ...
    describe('GQL API', () => {
            ...
            const { createTask } = await utils.gqlCreateTask(
                {
                    ...
                    statusId: projectTasksStatuses.find(
                        (i: any) => i.code === 'OPENED',
                    ).id,
                },
                adminAccessToken,
        ...
        it('Must return task', async () => {
            ...
            const taskPayloadStatus = projectTasksStatuses.find(
                (i: any) => i.code === 'OPENED',
            )
    ...
    describe('GQL API Task Update', () => {
        ...
        beforeAll(async () => {
            ...
            const gqlProjectTasksTags = await utils.gqlProjectTasksTags({
                projectId: createProject,
            })
        ...
        it('Must update task tags', async () => {
            ...
            const tagIdsOnCreate = projectTasksTags
                .filter((i) => ['TASK', 'BUG'].includes(i.code))
                .map((i: any) => i.id)
            const tagIdsOnUpdate = projectTasksTags
                .filter((i) => ['BUG', 'STORY'].includes(i.code))
                .map((i: any) => i.id)
    ...
    describe('GQL API Tasks', () => {
        ...
        let taskPayload_2: TaskIntegrationCreateTaskInput = {} as any
        let taskPayload_3: TaskIntegrationCreateTaskInput = {} as any
        ...
        beforeAll(async () => {
            ...
            taskPayload_3 = {
                ...
                estimatedDuration: 3,
                tagIds: projectTasksTags
                    .filter((i) => ['STORY', 'TASK'].includes(i.code))
                    .map((i: any) => i.id),
            }
        ...
        it('Must return tasks of current user', async () => {
            ...
            const { myTasks } = await utils.gqlMyTasks(undefined, accessToken)

            expect(myTasks).toBeDefined()
            expect(myTasks.data).toBeDefined()
            expect(myTasks.meta).toBeDefined()
            expect(myTasks.data).toHaveLength(2)
        })
        ...
        it('Must return filtered tasks by name', async () => {
            ...
            expect(tasks?.data?.map((i) => i.name)).toEqual([
                taskPayload_2.name,
                taskPayload_1.name,
            ])
        })
        ...
        it('Must return filtered tasks by tag id', async () => {
            const { tasks } = await utils.gqlTasks({
                filterBy: {
                    tagId: projectTasksTags
                        .filter((i) => ['TASK'].includes(i.code))
                        .map((i: any) => i.id),
                },
            })
        ...
        it('Must return filtered tasks by status', async () => {
            const { tasks } = await utils.gqlTasks({
                filterBy: {
                    statusId: projectTasksStatuses
                        .filter((i) =>
                            ['OPENED', 'IN_PROGRESS'].includes(i.code),
                        )
                        .map((i: any) => i.id),
                },
            })
        ...
        it('Must return filtered tasks by full text', async () => {
            const { tasks: tasks_1_by_name } = await utils.gqlTasks({
                filterBy: { fulltext: ['lorem iPsum'] },
            })
            const { tasks: tasks_1_by_description } = await utils.gqlTasks({
                filterBy: { fulltext: ['ConsectEtur'] },
            })
            const { tasks: tasks_by_name } = await utils.gqlTasks({
                filterBy: { fulltext: ['loRem'] },
            })
            const { tasks: tasks_by_description } = await utils.gqlTasks({
                filterBy: { fulltext: ['doLor'] },
            })
            const { tasks: tasks_by_different } = await utils.gqlTasks({
                filterBy: { fulltext: ['veLit'] },
            })
            const { tasks: tasks_by_name_combined } = await utils.gqlTasks({
                filterBy: { fulltext: ['pariaTUr', 'temPora'] },
            })
            const { tasks: tasks_by_descriptions_combined } =
                await utils.gqlTasks({
                    filterBy: { fulltext: ['consectetur', 'natus'] },
                })

            expect(tasks_1_by_name.data.map((i) => i.name)).toEqual([
                taskPayload_1.name,
            ])
            expect(tasks_1_by_description.data.map((i) => i.name)).toEqual([
                taskPayload_1.name,
            ])
            expect(tasks_by_name.data.map((i) => i.name)).toEqual([
                taskPayload_2.name,
                taskPayload_1.name,
            ])
            expect(tasks_by_description.data.map((i) => i.name)).toEqual([
                taskPayload_2.name,
                taskPayload_1.name,
            ])
            expect(tasks_by_different.data.map((i) => i.name)).toEqual([
                taskPayload_2.name,
                taskPayload_1.name,
            ])
            expect(tasks_by_name_combined.data.map((i) => i.name)).toEqual([
                taskPayload_2.name,
                taskPayload_1.name,
            ])
            expect(
                tasks_by_descriptions_combined.data.map((i) => i.name),
        ...
        it('Must return filtered tasks by created at', async () => {
            const { tasks: tasks_1 } = await utils.gqlTasks({
                filterBy: {
                    createdAtFrom: Number(createdAtBefore_2),
                },
            })
            const { tasks: tasks_2 } = await utils.gqlTasks({
                filterBy: {
                    createdAtTo: Number(createdAtAfter_2),
                },
            })

            expect(tasks_1?.data?.map((i) => i.name)).toEqual([
                taskPayload_3.name,
                taskPayload_2.name,
            ])
            expect(tasks_2?.data?.map((i) => i.name)).toEqual([
                taskPayload_2.name,
                taskPayload_1.name,
            ])
        })
    ...
    describe('GQL API Errors', () => {
        ...
        it('Must not update task', async () => {
            ...
            expect(conflictingProps.statusId.old).toBe(
                projectTasksStatuses_2.id,
            )
            expect(conflictingProps.statusId.new).toBe(
                projectTasksStatuses_3.id,
            )
