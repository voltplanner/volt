...
import { DEFAULT_USER_ROLE_CODES,DEFAULT_USERS_ROLES } from "../constants/project-integration-default-users-roles.constant";
        ...
        for (const roleCode of getObjectValues(DEFAULT_USER_ROLE_CODES)) {
            ...
            for (const actionCode of getObjectKeys(DEFAULT_USERS_ACTIONS)) {
                ...
                await this._taskUserService.actionUpsert({
                    ...
                    isAllowed: DEFAULT_USERS_PERMISSIONS[roleCode][actionCode],
                }, client)
            }
        }
    }

    async initProjectTasksStatuses(dto: {
        projectId: string
    }, client?: PrismaTransactionClientType) {
        const { projectId } = dto

        for (const item of DEFAULT_TASKS_STATUSES.map(i => ({ ...i, projectId }))) {
            await this._taskProjectService.tasksStatusesUpsert(item, client)
        }
    }

    async initProjectTasksTags(dto: {
        projectId: string
    }, client?: PrismaTransactionClientType) {
        const { projectId } = dto

        for (const item of DEFAULT_TASKS_TAGS.map(i => ({ ...i, projectId }))) {
            await this._taskProjectService.tasksTagsUpsert(item, client)
        }
    }

    async initProjectTasksRelations(dto: {
        projectId: string
    }, client?: PrismaTransactionClientType) {
        const { projectId } = dto

        for (const item of DEFAULT_TASKS_RELATIONS.map(i => ({ ...i, projectId }))) {
