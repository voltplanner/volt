...
export class TasksService {
    ...
    async create(dto: {
        ...
        const { name, typeId, statusId, projectId, createdById, parentId } = dto
        ...
        if (parentId) {
            ...
            const result = await this._prisma.task.aggregate({
                _max: { rgt: true },
            })
        ...
        // TODO: Made the field serial in the DB
        const maxNumber = await this._prisma.task.aggregate({
            _max: { number: true },
        })
    ...
    async update(dto: { id: string; parentId: string }): Promise<string> {
        ...
        const task = await this._prisma.task.findUniqueOrThrow({
            where: { id },
        })
